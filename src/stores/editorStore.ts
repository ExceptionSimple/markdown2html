...
微信公众号后台会彻底过滤 svg 和脚本，直接粘贴会变成空白。本应用在导出和预览时，会自动将 Mermaid 流程图与时序图绘制到高分屏 Canvas，转换为 **PNG 高保真图片** 嵌入 HTML，确保微信后台 100% 完整显示！
